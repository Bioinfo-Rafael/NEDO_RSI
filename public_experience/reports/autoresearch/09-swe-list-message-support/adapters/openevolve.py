...
class OpenEvolve(Adapter):
    ...
    def unbound_events(self):
        for p in sorted((ROOT/'raw/openevolve/issue_attachments').glob('*/*')):
            if p.suffix=='.json':
                yield event(self.source,None,None,p,1,json.loads(p.read_text()),'issue_checkpoint_without_run',0)
            else:
                for i,line in enumerate(p.read_text().splitlines(),1):
                    yield event(self.source,None,None,p,i,{'line':line},'issue_log_without_run',i)
    def bundles(self):
