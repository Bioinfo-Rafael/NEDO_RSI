...
class SearchAgents(Adapter):
    ...
    def bundles(self):
        for p in self.discover():
            ...
            yield finish(self.source,rid,ns,es,dataset='kohjingyu/search-agents/GPT-4o',structure='partial_search_tree',reference=relative(p),metadata={'task_name':task,'incomplete':True,'unsaved_branch_observations':True})
