...
from Monitor.direct_trace import AppendTracePayload, TraceQuery, append_node, read_trace


async def append_trace_node(payload: AppendTracePayload) -> Dict[str, Any]:
    """Record one completed, verifiable step using caller-authored DAG data; no LLM/API.

    Use this instead of build_trace in API-free workflows. Call get_trace to find
    parent IDs. First node: parents=[]; later nodes: one or more existing parents.
    GoT assigns the ID and root self-edge; never supply node.id. Parallel ablations
    share their baseline parent; comparisons reference all relevant experiments.
    Include artifact paths and measured metrics. Skip planning and trivial typo fixes.
    Returns assigned id, parents and output path. Validation errors are MCP errors.
    No deduplication of repeated calls: after an uncertain response, read get_trace
    before retrying to avoid recording the same deliverable twice.
    """
    if isinstance(payload, dict):
        payload = AppendTracePayload.model_validate(payload)
    return await append_node(payload)


async def get_trace(payload: TraceQuery) -> Dict[str, Any]:
    """Read a project's session DAG: id, title, parents, status. No LLM/network.

    Returns nodes=[] for a new session. Use before choosing parents or retrying an
    append whose outcome is uncertain. Does not create or modify any trace file.
    """
    if isinstance(payload, dict):
        payload = TraceQuery.model_validate(payload)
    return await read_trace(payload)
