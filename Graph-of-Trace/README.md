...
### Codex-authored nodes (no extraction API)

This checkout also exposes `append_trace_node` and `get_trace`. They use the
existing output configuration, file lock and atomic writer without loading an
LLM adapter. `build_trace` remains available with its original extraction behavior.

Both new MCP tools accept a `payload` argument. To append a root:

```json
{"payload":{"project":{"name":"nedo-rsi"},"session":{"id":"run-001"},"node":{"title":"Baseline","description":"Completed baseline; seed=42, accuracy=0.8, artifact: results/metrics.json","status":"completed","parents":[],"artifacts":[{"path":"results/metrics.json","type":"json"}]}}}
```

GoT assigns `N001` and its root self-reference. Subsequent nodes must supply
existing parent IDs, e.g. `[{"id":"N001","relation":"necessitated_by"}]`.
Optional parent `explanation` is preserved. Never supply `node.id`.
The response includes `status`, assigned `id`, normalized `parents`, and `path`.
Duplicate parent edges are removed. Unknown parents, invalid IDs, corrupt files,
project/session collisions and existing cycles produce errors without replacing
the existing trace. IDs are allocated under the same cross-process lock used by
`build_trace`. Direct mode records the first supplied node as root, without an
extra synthetic Session start node.

Read a compact snapshot using `get_trace`:

```json
{"payload":{"project":{"name":"nedo-rsi"},"session":{"id":"run-001"}}}
```

Returns `meta` and `nodes` containing `id`, `title`, `parents`, and `status`;
an unused session returns `nodes: []`. Appends are not idempotent: if a response
is lost, inspect the snapshot before retrying. The existing frontend and schema
are unchanged. `tests/test_direct_mcp.py` tests the real stdio server with internet
sockets and LLM imports forbidden, and creates an API-free smoke DAG when run
directly. `node tests/test_direct_viewer.mjs` checks that DAG using the actual
frontend converter and layout (Node >=22.18).

The following provider configuration is needed **only for legacy `build_trace`**.
